<div x-data="{ 
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
            <span class="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-700 rounded-full text-xs font-bold border border-emerald-100">
                <span class="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></span>
                Live Analytics
            </span>
            <button class="p-2 md:px-4 md:py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-xl text-slate-600 dark:text-slate-300 text-sm font-semibold hover:bg-slate-50 dark:bg-slate-700/50 transition-colors flex items-center gap-2 shadow-sm">
                <svg class="w-4 h-4R" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-white dark:bg-slate-800 p-5 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow">
        ...
        <div class="bg-white dark:bg-slate-800 p-5 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow border-l-4 border-l-emerald-500">
            <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Avg Ton/Ha</p>
            <div class="flex items-end justify-between">
                <h3 class="text-2xl font-black text-slate-800 dark:text-slate-100">{{ number_format($summary['avg_ton_per_ha'], 1) }}</h3>
                <div class="flex flex-col items-end">
                    <span class="text-[10px] text-emerald-600 font-bold">+2.4% yield</span>
                </div>
            </div>
        </div>
        <div class="bg-white dark:bg-slate-800 p-5 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow border-l-4 border-l-amber-500">
            <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Best Yield</p>
            <div class="flex items-end justify-between">
                <h3 class="text-2xl font-black text-slate-800 dark:text-slate-100">{{ $summary['best_yield_block'] }}</h3>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-4">
            <div class="bg-white dark:bg-slate-800 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden relative" style="height: 500px;">
                <div id="map" class="w-full h-full" wire:ignore></div>
                
                <!-- Map Overlay Legend -->
                <div class="absolute bottom-6 right-6 p-4 bg-white/90 dark:bg-slate-800/90 backdrop-blur-md rounded-2xl shadow-xl border border-white/50 z-10">
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Indikator Kesuburan</p>
                    <div class="space-y-2">
                        <div class="flex items-center gap-3">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-slate-800 p-6 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-700">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h4 class="text-sm font-black text-slate-800 dark:text-slate-100 uppercase tracking-tight">Tren Panen 12 Bulan</h4>
            ...
            <div class="bg-white dark:bg-slate-800 p-6 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-700">
                <h4 class="text-sm font-black text-slate-800 dark:text-slate-100 uppercase tracking-tight mb-6">Distribusi Kesuburan</h4>
                <div class="space-y-4">
                    @foreach($summary['distribution'] as $status => $count)
                        ...
                        <div class="space-y-1.5">
                            <div class="flex justify-between text-[11px] font-bold">
                                <span class="text-slate-600 dark:text-slate-300 uppercase tracking-wider">{{ $status }}</span>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
        <div class="p-8 border-b border-slate-50 dark:border-slate-700 flex items-center justify-between">
            <h4 class="text-sm font-black text-slate-800 dark:text-slate-100 uppercase tracking-tight">Status Operasional Per Blok</h4>
            <div class="flex gap-2">
                <input type="text" placeholder="Cari blok..." class="text-[11px] font-bold px-4 py-2 border border-slate-200 dark:border-slate-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20">
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-slate-800/50">
                        <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Identitas Blok</th>
                        <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Umur Tanaman</th>
                        <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Status</th>
                        <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Ton/Ha</th>
                        <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Total Est. Year</th>
                        <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right"></th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-50">
                    @foreach($blocks as $block)
                    <tr class="group hover:bg-slate-50 dark:bg-slate-700/50 transition-colors cursor-pointer" @click="selectedBlock = {{ json_encode($block) }}; setTimeout(() => window.initDetailChart(selectedBlock), 100);">
                        <td class="px-8 py-5">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold bg-{{ $block['color_name'] }}-500 shadow-lg shadow-{{ $block['color_name'] }}-100">
                                    {{ explode(' ', $block['name'])[1] }}
                                </div>
                                <div>
                                    <p class="text-[13px] font-bold text-slate-800 dark:text-slate-100">{{ $block['name'] }}</p>
                        ...
                        <td class="px-8 py-5 text-center">
                            <span class="text-[11px] font-bold text-slate-600 dark:text-slate-300 italic">
                                ...
                                <span class="ml-1 text-[9px] text-slate-400 uppercase">({{ substr($block['prediction']['status_label'], 0, 5) }}..)</span>
                            </span>
                        </td>
                        <td class="px-8 py-5 text-center">
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold bg-{{ $block['color_name'] }}-50 text-{{ $block['color_name'] }}-700 border border-{{ $block['color_name'] }}-100">
                                <span class="w-1.5 h-1.5 rounded-full bg-{{ $block['color_name'] }}-500"></span>
                                {{ $block['status'] }}
                            </span>
                        </td>
                        <td class="px-8 py-5 text-right font-black text-[13px] text-slate-800 dark:text-slate-100">{{ $block['ton_per_ha'] }}</td>
                        <td class="px-8 py-5 text-right">
                             <div class="flex flex-col items-end">
                                <span class="text-[13px] font-black text-slate-800 dark:text-slate-100">{{ number_format($block['yield']) }}</span>
                                <span class="text-[9px] text-slate-400 font-bold uppercase tracking-widest">TON</span>
                             </div>
                        </td>
                        <td class="px-8 py-5 text-right">
                            <div class="w-8 h-8 rounded-full flex items-center justify-center bg-slate-100 text-slate-400 group-hover:bg-slate-900 group-hover:text-white transition-all">
    ...
    <template x-if="selectedBlock">
        <div class="fixed inset-0 z-[100] flex items-center justify-end p-4 bg-slate-900/40 backdrop-blur-sm transition-all duration-300"
            ...
            <div x-show="selectedBlock" 
                ...
                <!-- Close Button -->
                <button @click="closeDetail()" class="absolute top-6 right-6 p-2 rounded-full bg-slate-100 text-slate-500 hover:bg-slate-200 transition-colors z-20">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>

                <!-- Header -->
                <div class="p-8 pb-6 bg-gradient-to-br from-slate-50 to-white border-b border-slate-100 dark:border-slate-700">
                    <div class="flex items-center gap-3 mb-4">
                        <div class="w-12 h-12 rounded-2xl flex items-center justify-center text-white font-bold"
                             :class="'bg-'+selectedBlock.color_name+'-500 shadow-lg shadow-'+selectedBlock.color_name+'-100'">
                            <span x-text="selectedBlock.name.split(' ')[1]"></span>
                        </div>
                        <div>
                            <h2 class="text-2xl font-extrabold text-slate-800 dark:text-slate-100" x-text="selectedBlock.name"></h2>
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest" x-text="'Plant age ' + selectedBlock.prediction.age_years + ' Yrs | ' + selectedBlock.area_ha + ' Ha'"></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-8 space-y-8">
                    ...
                    <div class="p-6 bg-slate-900 rounded-[2rem] text-white relative overflow-hidden">
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3">Estimasi Hasil Panen Tahunan</p>
                        <div class="flex items-end gap-2">
                            <h4 class="text-4xl font-extrabold" x-text="parseFloat(selectedBlock.yield).toLocaleString()"></h4>
                        ...
                        <div class="mt-4 pt-4 border-t border-white/10 flex justify-between">
                            ...
                            <div class="text-right">
                                ...
                                <p class="text-sm font-bold text-emerald-400" x-text="selectedBlock.prediction.status_label"></p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <p class="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-4">Prediksi Panen Bulanan</p>
                    ...
                    <!-- Nutrients Grid -->
                    <div>
                        <p class="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-3">Kondisi Hara Terakhir</p>
                        <div class="grid grid-cols-2 gap-2">
                            <div class="bg-slate-50 dark:bg-slate-700/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 transition-all hover:bg-white dark:bg-slate-800 hover:shadow-md">
                                <p class="text-[9px] font-bold text-slate-400 uppercase mb-1">Nitrogen (N)</p>
                                <p class="text-base font-extrabold text-slate-800 dark:text-slate-100" x-text="selectedBlock.raw_nutrients.nitrogen + '%'"></p>
                            </div>
                            <div class="bg-slate-50 dark:bg-slate-700/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 transition-all hover:bg-white dark:bg-slate-800 hover:shadow-md">
                                <p class="text-[9px] font-bold text-slate-400 uppercase mb-1">Fosfor (P)</p>
                                <p class="text-base font-extrabold text-slate-800 dark:text-slate-100" x-text="selectedBlock.raw_nutrients.phosphorus + ' ppm'"></p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6 bg-emerald-50 rounded-2xl border border-emerald-100">
                        <h5 class="text-xs font-black text-emerald-800 uppercase tracking-widest mb-2 flex items-center gap-2">
                ...
                <div class="p-8 border-t border-slate-50 dark:border-slate-700 bg-slate-50/30">
                    <button class="w-full bg-emerald-500 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-emerald-200 hover:bg-emerald-600 transition-all active:scale-[0.98]">
...
<script>
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        ...
                        bodyFont: { size: 11, weight: 'bold', family: 'sans-serif' },
                        padding: 10,
                        cornerRadius: 8,
                        displayColors: false,
                        callbacks: {
    ...
    function initMap() {
        ...
        const blocks = @json($blocks).map(b => ({
            ...
            name: b.name,
            coords: b.coords,
            color: b.color_name === 'emerald' ? '#10b981' : (b.color_name === 'amber' ? '#fbbf24' : '#f43f5e'),
            rawData: b
        }));
        ...
        blocks.forEach(block => {
            if (block.coords && Array.isArray(block.coords) && block.coords.length > 0) {
                ...
                polygon.setStyle(new ol.style.Style({
                    stroke: new ol.style.Stroke({ color: block.color, width: 3 }),
                    fill: new ol.style.Fill({ color: block.color + '44' }) // 27% alpha
                }));
        ...
        tooltip.className = "bg-slate-900 px-3 py-1.5 rounded-lg shadow-2xl text-[11px] font-bold text-white pointer-events-none z-50";
        ...
        window.detailChartInstance = new Chart(ctx, {
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        backgroundColor: '#0f172a',
                        titleFont: { size: 9, family: 'sans-serif' },
                        bodyFont: { size: 10, weight: 'bold', family: 'sans-serif' },
                        padding: 8,
                        cornerRadius: 6,
                        displayColors: false,
                        callbacks: {
