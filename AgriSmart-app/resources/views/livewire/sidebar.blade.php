{{-- ===== SIDEBAR (Updated: Profile Info clickable) ===== --}}
<div>
    <aside :class="open ? 'sidebar-width-open' : 'sidebar-width-close'"
        ...
        <div class="relative flex items-center px-6 py-6 border-b border-slate-50 dark:border-slate-800">
            <div class="flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center bg-emerald-500 shadow-lg shadow-emerald-200 dark:shadow-emerald-900/50">
            ...
            <div x-show="open" x-transition:enter="transition duration-150 delay-100"
                ...
                <p class="text-emerald-600 dark:text-emerald-400 text-[10px] font-bold uppercase tracking-widest">Palm Oil GIS</p>
            </div>
        </div>
        ...
        <nav class="flex-1 overflow-y-auto overflow-x-hidden py-6 space-y-1.5 px-4">
            ...
            @php
                $menus = [
                    ...
                    ['id' => 'analytics',   'label' => 'Analisis Kesuburan','route' => 'analytics',   'icon' => 'M13 7h8m0 0v8m0-8l-8 8-4-4-6 6'],
                    ...
                    ['id' => 'setting',     'label' => 'Setting',           'route' => 'setting',     'icon' => 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z'],
                ];
            @endphp
            ...
            @foreach($menus as $menu)
                <a href="{{ route($menu['route']) }}" wire:navigate
                    ...
                    <div class="{{ request()->routeIs($menu['route']) ? 'text-emerald-600 dark:text-emerald-500' : 'text-slate-400 group-hover:text-emerald-500 dark:group-hover:text-emerald-400' }} flex-shrink-0 transition-all duration-200">
        ...
        <div class="p-4 border-t border-slate-100 dark:border-slate-800 space-y-2">
            ...
            <!-- Profile Info → Navigates to Profile page -->
            <a href="{{ route('profile') }}" wire:navigate
                :class="open ? 'px-2' : 'justify-center'"
                class="group flex items-center gap-3 py-2 rounded-2xl transition-all duration-200
                    {{ request()->routeIs('profile') ? 'bg-emerald-50 dark:bg-emerald-900/30 ring-1 ring-emerald-100 dark:ring-emerald-900' : 'hover:bg-slate-50 dark:hover:bg-slate-800' }}
                    cursor-pointer">

                <div class="relative flex-shrink-0">
                    <div class="w-10 h-10 rounded-xl flex items-center justify-center bg-gradient-to-br from-emerald-400 to-emerald-600 text-white font-bold text-sm shadow-md shadow-emerald-100 dark:shadow-emerald-900/50 italic
                        group-hover:shadow-emerald-200 dark:group-hover:shadow-emerald-900/70 transition-shadow duration-200">
                        {{ strtoupper(substr(Auth::user()->username ?? 'AD', 0, 2)) }}
                    </div>
                    {{-- Online indicator --}}
                    <span class="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white dark:border-slate-900 bg-emerald-500"></span>
                </div>

                <div x-show="open" class="flex-1 min-w-0">
                    <p class="text-slate-800 dark:text-slate-200 text-sm font-bold truncate
                        {{ request()->routeIs('profile') ? 'text-emerald-700 dark:text-emerald-400' : '' }}">
                        {{ Auth::user()->username ?? 'Administrator' }}
                    </p>
                    <p class="text-slate-400 dark:text-slate-500 text-[10px] font-medium uppercase tracking-tighter truncate">
                        {{ Auth::user()->role ?? 'Estate Manager' }}
                    </p>
                </div>

                {{-- Arrow hint on hover --}}
                <div x-show="open" class="flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity duration-200
                    {{ request()->routeIs('profile') ? '!opacity-100 text-emerald-500' : 'text-slate-300 dark:text-slate-600' }}">
    ...
    <!-- Toggle Button -->
    <button @click="open = !open" :class="open ? 'left-[200px]' : 'left-[67px]'"
